...
describe('PdfZoomInComponent', () => {
  ...
  describe('eventListener - disable logic', () => {
    ...
    it('should disable when scale >= maxZoom', () => {
      eventBus.dispatch('scalechanging', { source: { maxZoom: 5 }, scale: 5 });
      expect(component.disabled).toBe(true);

      eventBus.dispatch('scalechanging', { source: { maxZoom: 5 }, scale: 6 });
      expect(component.disabled).toBe(true);
    });

    it('should enable when scale < maxZoom', () => {
      eventBus.dispatch('scalechanging', { source: { maxZoom: 5 }, scale: 4.9 });
      expect(component.disabled).toBe(false);
    });

    it('should enable when maxZoom is falsy (0 or undefined)', () => {
      eventBus.dispatch('scalechanging', { source: { maxZoom: 0 }, scale: 100 });
      expect(component.disabled).toBe(false);

      eventBus.dispatch('scalechanging', { source: { maxZoom: undefined }, scale: 100 });
      expect(component.disabled).toBe(false);
    });

    it('should stop firing after ngOnDestroy', () => {
      eventBus.dispatch('scalechanging', { source: { maxZoom: 5 }, scale: 4 });
      expect(component.disabled).toBe(false);

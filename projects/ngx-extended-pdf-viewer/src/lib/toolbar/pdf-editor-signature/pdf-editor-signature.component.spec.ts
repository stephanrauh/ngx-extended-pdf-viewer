...
describe('PdfEditorSignatureComponent', () => {
  ...
  describe('AbortController lifecycle (#3135)', () => {
    ...
    it('should unsubscribe handler on ngOnDestroy so it stops firing', () => {
      ...
      eventBus.dispatch('annotationeditormodechanged', { mode: 101 });
  ...
  describe('annotationeditormodechanged handler', () => {
    it('should set isSelected when mode matches SIGNATURE (101)', () => {
      jest.useFakeTimers();
      initPdfViewer();
  ...
  describe('onClick', () => {
    ...
    it('should dispatch switchannotationeditormode', () => {
      ...
      expect(eventBus.dispatch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ mode: AnnotationEditorType.SIGNATURE })
      );
    });

    it('should toggle to NONE when already in SIGNATURE mode', () => {
      mockPDFViewerApplication.pdfViewer.annotationEditorMode = AnnotationEditorType.SIGNATURE;
      component.onClick({ detail: 1 } as any);
      expect(eventBus.dispatch).toHaveBeenCalledWith(
        'switchannotationeditormode',

...
function findActiveRules(el = document.getElementsByTagName("ngx-extended-pdf-viewer")[0], rules = []) {
  for (const sub of el.children) {
    const additionalRules = css(sub);
    if (additionalRules) {
      for (const r of additionalRules) {
        if (!rules.includes(r)) {
          rules.push(r);
        }
      }
    }
    findActiveRules(sub, rules);
  }
  return rules;
}
function findEveryCSSRule() {
  const rules = findActiveRules();
  let sourcecode = "";
...
window.css = css;
window.findEveryCSSRule = findEveryCSSRule;

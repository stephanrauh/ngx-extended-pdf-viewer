...
function webViewerWheel(evt) {
  ...
  if (isPinchToZoom || evt.ctrlKey && supportsMouseWheelZoomCtrlKey || evt.metaKey && supportsMouseWheelZoomMetaKey) {
    ...
    if (isPinchToZoom && supportsPinchToZoom) {
      scaleFactor = app_PDFViewerApplication._accumulateFactor(previousScale, scaleFactor, "_wheelUnusedFactor");
      if (scaleFactor < 1) {
        app_PDFViewerApplication.zoomOut(null, scaleFactor);
      } else if (scaleFactor > 1) {
        app_PDFViewerApplication.zoomIn(null, scaleFactor);
      } else {
        return;
      }
    } else {
      ...
      if (ticks < 0) {
        app_PDFViewerApplication.zoomOut(-ticks);
      } else if (ticks > 0) {
        app_PDFViewerApplication.zoomIn(ticks);
      } else {
        return;
      }
    }
    app_PDFViewerApplication._centerAtPos(previousScale, evt.clientX, evt.clientY);
  }
}
...
function webViewerTouchMove(evt) {
  ...
  if (supportsPinchToZoom) {
    const newScaleFactor = app_PDFViewerApplication._accumulateFactor(previousScale, distance / pDistance, "_touchUnusedFactor");
    if (newScaleFactor < 1) {
      app_PDFViewerApplication.zoomOut(null, newScaleFactor);
    } else if (newScaleFactor > 1) {
      app_PDFViewerApplication.zoomIn(null, newScaleFactor);
    } else {
      return;
    }
  } else {
  ...
  app_PDFViewerApplication._centerAtPos(previousScale, (page0X + page1X) / 2, (page0Y + page1Y) / 2);
}

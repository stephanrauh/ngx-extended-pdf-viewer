...
- 21.0.0-alpha.9 #2377 removed pdf.js from the `window` object; use a sensible default value for `filenameForDownload`; stop throwing errors when closing a dialog that's already closed (happens sometimes when `ngOnChanges` fires); fixed form support after closing a viewer and opening a new one; #2363 fix an SVG rendering issue that happens only in Firefox

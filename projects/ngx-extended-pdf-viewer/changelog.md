...
- 26.0.0-alpha.0 **BREAKING CHANGE**: Updated to Angular 19 and TypeScript 5.6. Angular 18 exited its LTS phase, and CVE-2025-66035 won't be fixed in Angular 17 or 18. This update ensures continued security support. All components have been migrated to explicitly set `standalone: false` to maintain NgModule compatibility. Removed deprecated `async` testing utility (now synchronous in Angular 19). Updated zone.js to ~0.15.0. If you're using Angular 17 or 18, please stay on version 25.6.4 or earlier. #3105 fix the layout of the findbar; that's a breaking change because it drops the old checkboxes; Enabled TypeScript strict mode (`strict: true`, `noUnusedLocals`, `noUnusedParameters`, `noFallthroughCasesInSwitch`, `noImplicitOverride`, `forceConsistentCasingInFileNames`) for improved type safety and code quality. Removed all unused imports, variables, and parameters throughout the codebase. Fixed all type errors to ensure full TypeScript 5.6 compatibility; Migrated all `@Input()` and `@Output()` decorators to Angular signals (`input()`, `output()`, `model()`); Updated ng-add schematic to support Angular 21 with improved version detection (17-22) and informative messages about zone.js requirement; Fixed signal migration issues: added null checks for `printResolution` effect, added `destroyInitialization` guards to prevent emissions after component destruction, fixed querySelector errors in setZoom by adding proper null checks; **COMPLETE ZONELESS SUPPORT**: Fully implemented zoneless change detection support for Angular 21+ applications. Added `isZoneless()` and `asyncWithCD()` helper methods across all components that provide zero overhead in zone.js mode while enabling manual change detection in zoneless mode. Migrated ALL async operations library-wide: **Main component** (50+ async operations including all 29 PDF.js event handlers, initialization sequences, DOM manipulations, signal updates, and timing-critical operations), **15 toolbar button components** (pdf-book-mode, pdf-comment-editor, pdf-draw-editor, pdf-editor-signature, pdf-even-spread, pdf-highlight-editor, pdf-horizontal-scroll, pdf-infinite-scroll, pdf-no-spread, pdf-odd-spread, pdf-single-page-mode, pdf-stamp-editor, pdf-text-editor, pdf-vertical-scroll-button, pdf-wrapped-scroll-mode), **dynamic-css component** (infinite scroll height management), and **secondary-toolbar component** (UI state updates and visibility checks). The library now works seamlessly in both zone.js and zoneless Angular applications with full backward compatibility. Total: 73+ async operations migrated across all components. While zone.js is still supported, the library is now fully compatible with Angular's zoneless future; Fixed secondary menu visibility issue in zoneless mode: (1) Converted `NgxExtendedPdfViewerService.secondaryMenuIsEmpty` from a plain property to a signal for proper reactive change detection, (2) Wrapped MutationObserver callbacks in `asyncWithCD()` to trigger change detection when DOM mutations occur, (3) Added initial visibility check in `ngAfterViewInit()` to properly detect buttons after view initialization, fixing timing issues where the check ran before buttons were rendered

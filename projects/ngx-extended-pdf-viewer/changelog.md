...
- 26.0.0-alpha.1 #3131 fixed intermittent blank first page caused by pdf.js transferring (detaching) the ArrayBuffer to the worker thread; clone buffers before passing them to pdf.js and skip redundant re-opens when the src effect re-fires with the same value

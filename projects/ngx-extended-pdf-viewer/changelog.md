...
- 26.0.0-alpha.0 **BREAKING CHANGE**: Updated to Angular 19 and TypeScript 5.6.; #3105 fix the layout of the findbar; that's a breaking change because it drops the old checkboxes; Enabled TypeScript strict mode (`strict: true`, `noUnusedLocals`, `noUnusedParameters`, `noFallthroughCasesInSwitch`, `noImplicitOverride`, `forceConsistentCasingInFileNames`) for improved type safety and code quality. Migrated all `@Input()` and `@Output()` decorators to Angular signals (`input()`, `output()`, `model()`); Updated ng-add schematic to support Angular 21 with improved version detection (17-22) and informative messages about zone.js requirement; Added `isZoneless()` and `asyncWithCD()` helper methods across all components that provide zero overhead in zone.js mode while enabling manual change detection in zoneless mode; #3111 Fixed thumbnail rendering and scrolling issues after PDF.js v5.4.530 update. Root causes: (1) Flex container chain broken - pdf-sidebar-content and #viewsManager missing display:flex, (2) #thumbnailsView had conflicting position:absolute and overflow:auto creating dual scroll containers, (3) Thumbnail template used old anchor wrapper structure instead of direct div, (4) CSS used undefined variables --thumbnail-height, (5) Find bar select() called without safety check, (6) Find bar input field not rendering due to @if checking signal function instead of calling it. Fixes: Added flex container to #viewsManager and pdf-sidebar-content. Completely restructured #thumbnailsView from absolute positioned scrollable to flex layout matching PDF.js v5.4.530+. Removed anchor wrapper from thumbnail template. Updated .thumbnail to use inline-flex with auto sizing, removed float layout. Set --thumbnail-width on #thumbnailsView. Removed references to undefined --thumbnail-height variable. Added checkbox to template and hid it with CSS. Simplified mypdf.js to treat existingThumbnail as div directly. Added safety checks to findField.select() and findField.focus() in PDF find bar. Fixed pdf-find-input-area template to call customFindbarInputArea() signal instead of checking the function reference. Added zoneless support.

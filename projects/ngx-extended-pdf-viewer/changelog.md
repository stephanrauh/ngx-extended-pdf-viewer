...
- 29.0.0-rc.0 Updated the bleeding-edge engine to pdf.js 6.1. RichMedia and Screen annotations that play embedded audio or video now render (bleeding-edge). New `[supportsDownloading]` input: set it to `false` to hide the download and save buttons *and* disable the download manager entirely — a stronger lock than `[showDownloadButton]="false"`, which only hides the button (applies when the loaded engine is pdf.js 6.1 or newer). Checkboxes and radio buttons keep their familiar look: pdf.js 6.1 started drawing them onto a canvas over a tinted field background, which looked heavier; ngx keeps the plain box-and-checkmark / circle-and-dot rendering. #3239 the library now compiles in projects that enable `exactOptionalPropertyTypes: true`. Several public interfaces (e.g. `NgxHasHeight`, `PdfShyButtonDescription`, `PDFFindParameters`) now declare their optional properties as `T | undefined`. The library itself builds with the flag enabled, so this stays fixed. #3238 **breaking change:** when you commit a free-text annotation, the `annotationEditorEvent` that carries the typed text now has `type: 'textChanged'` instead of `type: 'commit'`. Previously two events fired with the same `type: 'commit'` — one from the base editor (where `value` is the editor itself) and one from the free-text editor (where `value` is the text string) — so you couldn't tell them apart. If you listen for `commit` to read the entered text, switch to `textChanged`; `commit` now consistently means "an annotation was committed" for every editor type. The `textChanged` event also gained the `editorType` field it was missing.

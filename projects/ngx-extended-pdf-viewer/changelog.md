...
- 28.0.0-rc.8 #3209 the derived asset options `workerSrc`, `cMapUrl`, `standardFontDataUrl`, and `sandboxBundleSrc` now resolve their relative paths against `document.baseURI` instead of emitting a bare `./assets/...` string. On apps served from a context path or using a `<base href>`, the relative worker URL was being re-based against the current route (or the base was applied twice), producing mangled URLs such as `https://appserver/crossdomainproxy//crossdomainproxy/../pdf.worker-x.y.z.min.mjs` and a failed worker load. This mirrors the earlier wasmUrl fix (#3140). Backward compatible: when the assets folder is an absolute URL, or there is no `document` (SSR), the exact previous string is returned; for ordinary root deployments the resolved absolute URL points at the same file the relative path already did. If you previously worked around this by setting `pdfDefaultOptions.workerSrc` manually, that override still wins and is unaffected.

...
- 28.1.0 #3229 in book mode, typing a page number into the toolbar's page field now turns to that page. Previously nothing happened — only the other navigation methods worked. #3234 `getPageAsCanvas()` and `getPageAsImage()` screenshots are now WYSIWYG. Four fixes: (1) they honour the page rotation you set in the viewer — a screenshot of a rotated page previously came out in the un-rotated orientation. (2) They now include annotations you just added with the editor (e.g. an image stamp) and current form-field values, by default — previously only annotations already saved into the PDF showed up, so a freshly stamped image was missing. Pass `annotationMode: AnnotationMode.ENABLE` if you want only the saved annotations. (3) Screenshotting a single annotation with `cropBox` now works even when the page is rotated. Use the editor's new rotation-independent `event.source.normalizedPageRect` as the `cropBox` (the raw `x`/`y`/`width`/`height` are stored in the rotation the annotation was added at, so they mis-cropped rotated annotations). (4) A new optional `rotation` parameter (`0` | `90` | `180` | `270`) overrides the user's rotation, e.g. pass `0` to always get the page in its authored orientation no matter how the user rotated it on screen.
- 28.1.1 **Security fix:** the bundled pdf.js is patched against [CVE-2026-16633](https://github.com/mozilla/pdf.js/security/advisories/GHSA-hq66-cqwq-w95j) (high) - a malicious PDF could run JavaScript in the context of your page. Mozilla fixed this in pdf.js 6.2.108; the fix is cherry-picked into the pdf.js 6.0 engine this release line uses, so nothing else changes. Note that this library ships `enableScripting=false` by default, unlike pdf.js itself, which limited the exposure - but not for the XFA part of the issue, so don't assume you were safe. Both the `assets/` and the `bleeding-edge/` bundle now carry engine 6.0.1174.
- 28.1.2 #3244 the package now contains an SBOM (`sbom.json`, CycloneDX), a `vex.json` and a `pdfjs-provenance.json` telling you which pdf.js is inside. ngx-extended-pdf-viewer bundles a fork of pdf.js rather than the `pdfjs-dist` npm package, so until now your dependency scanner had no way to see the PDF engine at all - and no way to tell you about a pdf.js CVE. The SBOM declares the engine with a `purl` and a CPE so Dependency-Track, Mend & co. can match advisories, and states openly that it is a patched fork of that upstream release. Because a patched engine keeps the version number of the pdf.js release it derives from, your scanner would otherwise keep reporting [CVE-2026-16633](https://github.com/mozilla/pdf.js/security/advisories/GHSA-hq66-cqwq-w95j) even though 28.1.1 already fixed it; the VEX says the fix is in. All three files are published in the npm package and attached to every GitHub release.

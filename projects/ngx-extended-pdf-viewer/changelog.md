...
- 26.0.0-rc.3 #3131 fixed intermittent blank first page: guarded the onCancel callback in the rendering pipeline to prevent a delayed cancellation from removing a canvas created by a subsequent draw() cycle; replaced setTimeout(setZoom()) with an awaited call after PDFViewerApplication.open() to eliminate a timing window where a zoom change could cancel in-progress first page rendering.

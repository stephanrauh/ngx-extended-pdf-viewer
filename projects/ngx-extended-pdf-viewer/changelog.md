...
- 27.0.0 #2943 fixed crash in bleeding-edge mode when enableSplitMerge is false (manageMenu null guard); updated the bleeding-edge version of pdf.js from 5.4.530 to 5.6.205
  - **Breaking changes:**
...
- 27.0.1 #3140 added [enableFlipByDrag] input for book mode: when false, drag gestures allow text selection and panning instead of flipping pages; when true (default), drag-to-flip is automatically disabled when zoomed in beyond page-fit scale; added [showPageFlipButton] for a three-mode cursor tool (select / hand / page flip) in book mode; added [showPageCorners] to control the dog-ear fold animation
  - **Breaking change:** The hand tool and select tool toolbar buttons now use `cursortoolchanged` events as the sole source of truth after pdf.js initialization, instead of the `[(handTool)]` two-way binding. This only matters if you programmatically set `[(handTool)]` after initialization and relied on the toolbar buttons updating immediately without going through pdf.js. In practice, this is unlikely to affect anyone.
- 27.0.2 #3140 fixed three cursor tool bugs: select button staying toggled when page-flip mode is active (toolbar components now read the initial cursor tool state on init); click-to-flip not working in select/hand mode (click detection separated from drag detection); hand tool drag not working in book mode (narrowed the `.stf__item` ignore check to only affect click targets inside page-flip elements). Also fixed `[src]="undefined"` causing an "Invalid PDF structure" error (guarded `openPDF2()` against falsy `_src`).

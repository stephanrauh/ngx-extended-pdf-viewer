...
- 28.0.1 #3219 follow-up to the 28.0.0 theme fix, for more surfaces that ignored the viewer theme and followed the OS: the comments sidebar, the add-comment dialog and comment popups stayed light with invisible text under a dark viewer theme — they now follow the theme you picked. Also, the per-annotation comment marker (the little speech-bubble next to a commented highlight) is now always light so it stays visible on top of the PDF page, instead of turning dark-on-dark and disappearing on dark-background PDFs under a dark OS.

...
- 26.0.0-rc.8 #3134 added showcase demo for auto-detected links; fixed the showcase documentation to use `(annotationLayerRendered)` instead of `(pageRendered)` for link manipulation; #3157 fixed unsmooth page scrolling caused by the signals migration — the `page` model signal's effect was snapping to the top of the page when the signal was updated by scroll events

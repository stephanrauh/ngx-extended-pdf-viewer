...
- 27.5.0-alpha.0 merged the latest pdf.js upstream (v5.7.284, ≈290 commits past v5.6.205) into the bleeding-edge branch, bringing the new merge-PDF UI and many bugfixes; new UI elements are gated by `pdfDefaultOptions.isBleedingEdge()`. The stable branch is still at v5.6.205 — once it's updated to v5.7.284 this will graduate to 27.5.0. Note: a number of historical mypdf.js commits are mis-attributed to "Test User" due to a stale local git config; all such commits were authored by Stephan Rauh (see the mypdf.js README). The local config has been corrected for future commits.
- 27.5.0-alpha.1 #3115 scoped the `color-scheme` CSS property to the `<ngx-extended-pdf-viewer>` host element instead of writing it to `document.documentElement`. Previously, `[theme]="'light'"` (the default) and `[theme]="'dark'"` polluted the page-wide `color-scheme`, which broke host-application theming (e.g. Angular Material auto/dark mode). Because `color-scheme` is an inherited CSS property, scoping it to the host still gives the viewer subtree (toolbar, sidebar, dialogs, `light-dark()` color resolution) the correct rendering, without leaking onto the surrounding application.; #1321 fixed `DOMException: Function object could not be cloned` when the viewer initialized: the `_showUnverifiedSignaturesEffect` read `this.showUnverifiedSignatures` without invoking the signal, so it sent the signal function itself to the worker via `postMessage` (structured clone rejects functions). As a side effect, the effect didn't track the signal either, so `[showUnverifiedSignatures]` changes after init were silently ignored. Adding the `()` fixes both. Also made `scrollIntoView()` tolerant of viewers that initialize while hidden (e.g. Ionic route transitions, lazy tabs): instead of dropping the scroll with an `offsetParent is not set` error, the request is now deferred via `IntersectionObserver` and replayed when the viewer becomes visible, so users no longer land on the wrong page after navigating between PDFs. A 60s watchdog and supersession by later scroll requests prevent the pending observer from leaking if the viewer is destroyed while still hidden.

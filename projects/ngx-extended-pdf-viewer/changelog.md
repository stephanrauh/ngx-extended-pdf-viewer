...
- 29.1.0 #3241 when a PDF fails to load, the viewer can show an error card on the empty page instead of leaving it blank. It's off by default — pdf.js only writes the error to the console and fires `(pdfLoadingFailed)` — so switch it on with `[showLoadingErrorMessage]="true"`. The card has a "More information" button revealing the technical details and a "Close" button, and it is translated into every language the viewer ships with. Use `[loadingErrorMessage]="'…'"` to replace the default headline with your own wording.

...
- 30.0.0-rc.0 #3253 an RTL language like Arabic now only flips the viewer, not your entire application. Until now the viewer switched the whole page to right-to-left, and the page stayed that way - horizontally scrolling - after the viewer had been destroyed. The viewer also stops writing its CSS variables to your `<html>` tag; they now live on the viewer itself, so nothing is left behind when you close it. **Breaking:** the viewer no longer touches `<html>` at all. If your CSS overrides the viewer with selectors like `html[dir='rtl'] .toolbarButton`, or if it reads `--viewer-container-height`, `--viewsManager-width` or `color-scheme` from `<html>`, use the viewer's own `.html` and `.body` elements instead. Custom viewer templates (`[customPdfViewer]`) need to keep those two wrapper elements. The bleeding-edge engine is pdf.js 6.2, which brings the digital signature properties panel: a signed PDF shows a toolbar button opening one card per signature, with a banner summarising the result. Hide it with `[showSignaturePropertiesButton]="false"`. Note that the browser build ships with signature verification switched off, so the panel reports what the document claims about itself.

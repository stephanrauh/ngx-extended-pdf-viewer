...
- 28.0.5 #3229 in book mode, typing a page number into the toolbar's page field now turns to that page. Previously nothing happened — only the other navigation methods worked. #3234 `getPageAsCanvas()` and `getPageAsImage()` screenshots are now WYSIWYG. Four fixes: (1) they honour the page rotation you set in the viewer — a screenshot of a rotated page previously came out in the un-rotated orientation. (2) They now include annotations you just added with the editor (e.g. an image stamp) and current form-field values, by default — previously only annotations already saved into the PDF showed up, so a freshly stamped image was missing. Pass `annotationMode: AnnotationMode.ENABLE` if you want only the saved annotations. (3) Screenshotting a single annotation with `cropBox` now works even when the page is rotated. Use the editor's new rotation-independent `event.source.normalizedPageRect` as the `cropBox` (the raw `x`/`y`/`width`/`height` are stored in the rotation the annotation was added at, so they mis-cropped rotated annotations). (4) A new optional `rotation` parameter (`0` | `90` | `180` | `270`) overrides the user's rotation, e.g. pass `0` to always get the page in its authored orientation no matter how the user rotated it on screen.

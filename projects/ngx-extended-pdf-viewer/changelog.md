...
- 23.0.0-alpha.8 #2736 added selectors to the CSS rules of dialogs to reduce interference with your own CSS rules (potentially breaking change if you've got sophisticated CSS rules); #2852 if the cursor is in the input field of the find bar, prevent the ESC key and CMD+cursor up/down to bubble
...
- 25.0.0-alpha.0 _Breaking Change:_ the license has changed from Apache 2.0 to Apache 2.0 with Commons Clause - a change you almost certainly won't mind; #2995 added `pdfDefaultOptions.enableCatalogAAJavaScript` to disable embedded JavaScript that runs when opening the PDF; added `pdfDefaultOptions.enableCatalogAAJavaScript` to disable embedded JavaScript that runs on the "additional actions" (/AA) WillClose (WC), WillSave (WS), DidSave (DS), WillPrint (WP), DidPrint (DP); both options are false by default; set `pdfDefaultOptions.enableScripting` to false; updated to pdf.js 5.4 (stable branch); #2997 fixed `[backgroundColor]`, a feature that was lost when updating to ngx-extended-pdf-viewer 24.2.5; make sure that the background color is the same background color that pdf.js uses when omitting the property [backgroundColor] (before the backgroundColor as accidentally set to #e8e8eb - a light gray - instead of the pure white default favored by pdf.js); #2998 restore the original `color-scheme` when destroying the `<ngx-extended-pdf-viewer>` component
...
- 25.6.0-rc.1 improved publishing automation; updated stable branch to pdf.js version 5.4.530
...
- 26.0.0-alpha.0 **BREAKING CHANGE**: Raised the minimum required version to Angular 19 and TypeScript 5.6.; #3110 added optional zoneless support; #3109 migrated to signals; #3113 activated the comment editor; #3111 updated to pdf.js 5.4.530, which causes breaking changes because Mozilla's modified the HTML tree of the PDF viewer and ngx-extended-pdf-viewer partially follows the changes
- 26.0.0-rc.1 #3131 fixed intermittent blank first page caused by pdf.js transferring (detaching) the ArrayBuffer to the worker thread; clone buffers before passing them to pdf.js and skip redundant re-opens when the src effect re-fires with the same value; unregister events ngx-extended-pdf-viewer registers with an AbortController, thus fixing "can't read properties of undefined (reading initialFormDataStoredInThePDF)" crash when the component is destroyed while annotation layers are still rendering; #3135 added AbortController-based eventBus listener cleanup to all child toolbar components (editor buttons, cursor tools, spread/scroll mode buttons, rotate/paging buttons, zoom buttons, secondary toolbar, sidebar content) to prevent listener leaks on component destroy; #3137 fixed `injectLinkAnnotations` race condition that threw "`render` method must be called before `injectLinkAnnotations`" when navigating between pages rapidly
- 26.0.0-rc.2 #3136 added `drawingStarted` and `drawingStopped` annotation editor events that fire when the user starts and stops actively drawing (ink/pencil) or highlighting; fixed custom toolbar page crash: changed `viewChild.required` to `viewChild` in `PdfShyButtonComponent` to support nested content mode (NG0951); added null check for `commentToolbarButton` in `CommentSidebar` to handle missing editor button when custom toolbar is used
...
- 26.0.0-rc.4 #3131 fixed intermittent blank first page; reduce the probability that the destructor of the old component destroys the data structures of the new component when switching between two instances of ngx-extended-pdf-viewer quickly
- 26.0.0-rc.5 #3116 fix: always serialize copied comments
- 26.0.0-rc.6 #3158 now radio buttons inherit their tooltip from the radio button group (unless they have a tooltip of their own); added a dark rim to radio buttons because unchecked radio buttons were almost invisible in some documents
- 26.0.0-rc.7 #3134 added `(linkAnnotationsAdded)` event that fires after auto-detected links (URLs found in text) are injected into the annotation layer
- 26.0.0-rc.8 #3134 added showcase demo for auto-detected links; fixed the showcase documentation to use `(annotationLayerRendered)` instead of `(pageRendered)` for link manipulation; #3157 fixed unsmooth page scrolling caused by the signals migration — the `page` model signal's effect was snapping to the top of the page when the signal was updated by scroll events; #3159 `[pdfBackgroundColor]` now reacts to changes at runtime (previously it was only applied at initialization); #3098 removed the redundant `shuttingDown` flag from `PdfScriptLoaderService` — all usages in the component now use the per-instance `destroyInitialization` flag instead, which correctly scopes cleanup to the component being destroyed; #3160 fixed table of contents navigation in book mode — clicking a TOC entry now uses PageFlip to turn to the correct page instead of trying to scroll the DOM (which has no effect in book mode); #3126 added error handling to the print service's `useRenderedPage()` — `toBlob()` failures and image load errors are now logged instead of failing silently with grey placeholders
...
- 26.0.0-rc.11 #3155 added `[readingDirection]` input (`'auto'` | `'ltr'` | `'rtl'`) for RTL reading support — `'auto'` (default) inherits from the UI language, `'ltr'` forces left-to-right, `'rtl'` forces right-to-left. In RTL mode, spread pages display right-to-left, arrow key navigation is reversed, and horizontal/wrapped scroll modes display pages right-to-left.
- 26.0.0-rc.12 #3155 added `[readingDirection]` input (`'ltr'` | `'rtl'`| `'auto'`)
- 26.0.0-rc.13 #3126 fixed Firefox print preview showing grey placeholders — blob URLs are now kept alive until after printing completes, and a `toDataURL()` fallback is used if `toBlob()` fails; #3069 fixed pinch zoom jumping to page 1 in infinite-scroll mode — zoom now adjusts the outer scroll container instead of the non-scrollable viewer container
- 26.0.0-rc.14 (burned - publish failed due to broken tests)
...
- 26.0.0-rc.26 #3069 fixed iPad pinch-zoom page jumping: during pinch/wheel zoom, save scrollTop before refresh()/CSS scale change, then restore it proportionally (savedScrollTop \* ratio) plus origin-based adjustment — bypasses scrollPageIntoView() and \_location which drift to wrong pages because refresh() causes unpredictable browser scroll recalculation (bleeding-edge only)
- 26.0.0-rc.27 #3069 (mostly) fixed iPad pinch-zoom scroll drift and stutter: freeze `_location` at gesture start so scroll events during the gesture cannot corrupt the scroll target; skip expensive `pageView.update()` work (viewport clone, computeScale, reset) for non-canvas pages during pinch — only visible pages get CSS-transformed; increase scale precision from 1% to 0.01% (10000 instead of 100) for smoother pinch increments; revert touch origin to native pdf.js `screenX/Y` coordinates; there's still a small drift towards other pages when pinching, but it's tolerable now (and I hope to solve it in future versions)
...
- 26.0.2 #3183 fixed viewer height in constrained containers (e.g. mat-card with height: 80vh): the signals migration removed a change guard, causing the pageViewMode effect to reset the height on initialization; also improved auto-height to respect parent overflow bounds; scroll the correct thumbnail into view ageain
- 27.0.0-rc.1 #2943 fixed crash in bleeding-edge mode when enableSplitMerge is false (manageMenu null guard); updated pdf.js from 5.4.530 to 5.6.205; #3140 added [enableFlipByDrag] input for book mode: when false, drag gestures allow text selection and panning instead of flipping pages; when true (default), drag-to-flip is automatically disabled when zoomed in beyond page-fit scale; added [showPageFlipButton] for a three-mode cursor tool (select / hand / page flip) in book mode; added [showPageCorners] to control the dog-ear fold animation; fixed three cursor tool bugs: select button staying toggled when page-flip mode is active (toolbar components now read the initial cursor tool state on init); click-to-flip not working in select/hand mode (click detection separated from drag detection); hand tool drag not working in book mode (narrowed the `.stf__item` ignore check to only affect click targets inside page-flip elements). Also fixed `[src]="undefined"` causing an "Invalid PDF structure" error (guarded `openPDF2()` against falsy `_src`).
- 27.0.0-rc.2 (failed to publish)
...
- 28.0.0-rc.4 fixed a Jest test that failed because of the changes to `<pdf-shy-button>`.
- 28.0.0-rc.5 Fixed a CSP violation (`script-src 'self'`) raised by the nested-content fallback in pdf-shy-button: the span had `onKeyDown="onClick($event)"` written as a raw HTML attribute instead of an Angular event binding, which (a) the browser tried to execute as an inline event handler and got blocked under strict CSPs, and (b) wouldn't have called the component's `onClick()` anyway because `onClick` was looked up on `window`, not on the component. Replaced with the proper Angular binding `(keydown)="onClick($event)"`. The buggy attribute was introduced on 2024-12-31 to silence a Sonarqube "missing keyboard handler" warning.
- 28.0.0-rc.6 Fixed compatibility with Safari <= 26.3
- 28.0.0-rc.7 #3216 fixed a crash when destroying a viewer and immediately creating a new one in its place. The new viewer no longer (less often?) throws `TypeError: Cannot read properties of undefined (reading 'dispatch')` while the previous viewer is still tearing down. Also made `NgxExtendedPdfViewerService.find()` / `findNext()` / `findPrevious()` more robust: they now check whether the find controller is actually ready, rather than trusting a flag that could be stale across viewer mounts. Regression introduced by the v27 signals migration; v25.6.4 was unaffected. Please note that this whole area is prone to error: pdf.js works asynchronously, and the architecture was global-heavy — pdf.js itself has moved away from that, but ngx-extended-pdf-viewer was built on top of the global style and I'm still untangling my own dependence on it. That'll take a while.
- 28.0.0-rc.8 #3209 fixed broken asset URLs (worker, CMaps, standard fonts, sandbox) when your app runs under a context path or `<base href>` — the paths no longer pick up a duplicated context segment and fail to load. If you worked around this by setting `pdfDefaultOptions.workerSrc` yourself, your override still wins. #2656 `addImageToAnnotationLayer()`, `addHighlightToAnnotationLayer()`, and `addEditorAnnotation()` no longer fail silently when called too early: wait for the target page's `(annotationEditorLayerRendered)` event — not `(annotationLayerRendered)` — before adding annotations. Calling too early now logs a clear warning instead of `Cannot read properties of undefined (reading 'deserialize')`. #3210 search highlights now appear on the first page in Safari, and the selected match stands out more clearly from the other matches.
- 28.0.0 #3219 fixed the outline (bookmarks) and the rest of the sidebar panel being hard to read when your OS color scheme didn't match the viewer theme — e.g. white text on a white background with a dark OS and the light theme. The sidebar now always follows the theme you picked, not the operating system.
- 28.0.1 #3219 the comments sidebar, the add-comment dialog and comment popups now follow your chosen theme instead of your OS, so they're no longer light with invisible text under the dark theme. The per-annotation comment marker (the speech-bubble next to a commented highlight) is now always light, so it stays visible even on dark PDF pages.
- 28.0.2 #3226 a new CSS variable `--page-placeholder-background-color` lets you remove the grey flash when you scroll to a page that hasn't rendered yet or switch PDFs — set it to `transparent` (or any color) on `ngx-extended-pdf-viewer`. #3225 annotations can now keep a stable id of your own across save and restore. Set a `customId` (e.g. a UUID) on a serialized annotation before you store it, and `addEditorAnnotation()` keeps it instead of replacing it — so `getSerializedAnnotations()` and the `annotationEditorEvent` give you back the same id every session. The built-in `id` is still a throwaway that changes frequently; use `customId` when you need to match annotations to your own records. Note that the `customId` never makes it into a downloaded / exported PDF document.
- 28.0.3 #3228 `getPageAsCanvas()` and `getPageAsImage()` now take an optional `cropBox` in normalized coordinates (between 0 and 1), so you can screenshot a single annotation by passing its rectangle straight in — no maths required.
- 28.0.4 #3232 JavaScript embedded in PDFs runs again. Since 28.0.0 it had silently stopped — for example, form fields that calculate their value from other fields no longer updated.
- 28.1.0 #3229 in book mode, typing a page number into the toolbar's page field now turns to that page. #3234 `getPageAsCanvas()` and `getPageAsImage()` are WYSIWYG now: they honour the rotation the user set, they include annotations you've just added with the editor and the current form-field values (pass `annotationMode: AnnotationMode.ENABLE` if you want only the annotations already stored in the file), and `cropBox` crops correctly on rotated pages - use the editor's new `event.source.normalizedPageRect` for that. A new optional `rotation` parameter (`0` | `90` | `180` | `270`) overrides the user's rotation, e.g. pass `0` to always get the page as it was authored.
- 29.0.0-rc.0 **Breaking change (#3238):** the `annotationEditorEvent` carrying the text of a free-text annotation now has `type: 'textChanged'` instead of `type: 'commit'`. Two events used to arrive as `commit` and you couldn't tell them apart; `commit` now always means "an annotation was committed". `textChanged` also has the `editorType` field it was missing. New service method `mergeDocument()` adds another PDF file or an image to the document on screen, at any position - including **before the first page**, which the sidebar's "Add file" button can't do. Pass a URL, a `File`, a `Blob`, the raw bytes or an `ImageBitmap`, plus optionally `insertAfterPage` (counting from 1; `0` puts the pages in front), `includePages`, `excludePages` and the `password` of an encrypted file. `deletePages()` removes pages by number, e.g. `deletePages([1, [8, 10]])`. Both resolve once the new document is on screen and keep pages the user reordered in the sidebar where they are. `extractPages()` is the low-level escape hatch: pdf.js's own (0-based) page descriptions, passed through unchanged. New `[supportsDownloading]="false"` hides the download and save buttons *and* switches the download manager off - a stronger lock than `[showDownloadButton]="false"`, which only hides the button (needs pdf.js 6.1 or newer). #3232 the CMaps and the standard fonts load again: `cMapUrl` and `standardFontDataUrl` pointed next to the assets folder instead of into it, so CJK documents lost their CMaps and files using one of the 14 standard fonts silently fell back to a substitute. If you set the two options yourself to work around this, you can drop that. #3239 the library compiles in projects with `exactOptionalPropertyTypes: true`. Public interfaces such as `NgxHasHeight`, `PdfShyButtonDescription` and `PDFFindParameters` now declare their optional properties as `T | undefined`. In book mode the hand tool now switches itself off, because it swallowed the drag gesture that turns the page; you get it back when you leave book mode. Previously this only happened with `[showPageFlipButton]="true"`. `addImageToAnnotationLayer()` and `addHighlightToAnnotationLayer()` no longer crash with "Failed to execute 'transferToImageBitmap'" when a coordinate lands outside the page - easy to hit with pixel values, which refer to the page as rendered. The rectangle is kept on the page, and if nothing is left to draw on you get an explanatory console message. The bleeding-edge engine is pdf.js 6.1 now: RichMedia and Screen annotations with embedded audio or video render, and checkboxes and radio buttons keep their familiar plain look instead of the heavier canvas-drawn style pdf.js 6.1 introduced.
- 29.0.0-rc.1 **pdf.js 6.1 is the stable engine now.** The `assets/` bundle ships pdf.js 6.1 instead of 6.0, so the features that were bleeding-edge-only in 29.0.0-rc.0 work out of the box: RichMedia and Screen annotations with embedded audio or video render, and `[supportsDownloading]="false"` takes effect. You no longer need `pdfDefaultOptions.assetsFolder = 'bleeding-edge'` for those. The bleeding-edge bundle keeps tracking Mozilla's development line; until upstream opens pdf.js 6.2 both bundles carry the same engine, so switching between them changes nothing for the time being. 
- 29.0.0-rc.2 #3244 the package now contains an SBOM (`sbom.json`, CycloneDX) and a `pdfjs-provenance.json` telling you which pdf.js is inside. ngx-extended-pdf-viewer bundles a fork of pdf.js rather than the `pdfjs-dist` npm package, so until now your dependency scanner had no way to see the PDF engine at all - and no way to tell you about a pdf.js CVE. The SBOM declares the engine with a `purl` and a CPE so Dependency-Track, Mend & co. can match advisories, and states openly that it is a patched fork of that upstream release. Both files are published in the npm package and attached to every GitHub release.
- 29.0.0-rc.3 **Security fix:** the bundled pdf.js is patched against [CVE-2026-16633](https://github.com/mozilla/pdf.js/security/advisories/GHSA-hq66-cqwq-w95j) (high) - a malicious PDF could run JavaScript in the context of your page. It affects pdf.js 5.6.83 and newer, so earlier 28.x and 29.0.0-rc releases are affected too; please update. Mozilla's fix (pdf.js 6.2.108) is cherry-picked into both the stable and the bleeding-edge engine. Note that this library ships `enableScripting=false` by default, unlike pdf.js itself, which limited the exposure - but not for the XFA part of the issue, so don't assume you were safe. The package now also contains `vex.json`: because the patched engine keeps the version number of the pdf.js release it derives from, your scanner would otherwise keep reporting this CVE. The VEX tells it the fix is already in, and `sbom.json` records the applied patch in its pedigree.
- 29.0.0-rc.4 test commit to verify the build pipeline still works
- 29.0.0-rc.5 #3242 the findbar's `showFind…` options take effect again - every option was shown no matter what you set, most visibly the regex search, which is off by default. The sidebar toolbar scales with `[mobileFriendlyZoomScale]` again. Changing `[customFindbar]`, `[customFindbarButtons]` or `[customFindbarInputArea]` while the viewer is open now updates it. And unless you set `[startTabindex]`, the viewer no longer renumbers the tab order of its buttons - it left the toolbar in a tab order you didn't ask for.
- 29.0.0 the final release of version 29 - unchanged since `29.0.0-rc.5`. In a nutshell: pdf.js 6.1 is the engine in both bundles, `mergeDocument()`, `deletePages()` and `extractPages()` let you build a document out of several files, `[supportsDownloading]="false"` switches downloading off for good, RichMedia and Screen annotations with embedded audio or video render, and the package ships an SBOM so your dependency scanner can see the bundled pdf.js. **Breaking (#3238):** the `annotationEditorEvent` carrying the text of a free-text annotation now has `type: 'textChanged'` instead of `type: 'commit'`.
- 29.0.1 #3255 the sidebar buttons are translated again - opening a PDF no longer floods the console with "[fluent] Missing translations" warnings for the thumbnail, outline, attachment, layer and toggle-sidebar buttons. Thumbnails in the sidebar carry their "Page x of y" tooltip again.
- 30.0.0-rc.0 #3253 an RTL language like Arabic now only flips the viewer, not your entire application. Until now the viewer switched the whole page to right-to-left, and the page stayed that way - horizontally scrolling - after the viewer had been destroyed. The viewer also stops writing its CSS variables to your `<html>` tag; they now live on the viewer itself, so nothing is left behind when you close it. **Breaking:** the viewer no longer touches `<html>` at all. If your CSS overrides the viewer with selectors like `html[dir='rtl'] .toolbarButton`, or if it reads `--viewer-container-height`, `--viewsManager-width` or `color-scheme` from `<html>`, use the viewer's own `.html` and `.body` elements instead. Custom viewer templates (`[customPdfViewer]`) need to keep those two wrapper elements. pdf.js 6.2 is the engine in both the stable and the bleeding-edge bundle. It brings the digital signature properties panel, which lists every signature of a signed PDF with a banner summarising the result. The panel needs something that can actually verify a signature, and the browser build of pdf.js ships none - so it stays hidden until you pass your own implementation to the new `[signatureVerifier]` input (Firefox uses NSS for this; a web page has no equivalent, and picking the root certificates to trust is a decision only your application can make). Hide the button entirely with `[showSignaturePropertiesButton]="false"`. Older browsers benefit too: the `-es5` bundle is now compiled for Chrome/Edge 80, Firefox 78, Safari 13.1 and iOS 13.4, and the switch that picks it now detects what the modern bundle really needs, so Safari 17.4 to 18.3 no longer loads a bundle it cannot run. On Angular 19 and 20 the viewer stayed empty: those versions still use zone.js, which replaces the browser's `Promise` with one that lacks `Promise.try` - and pdf.js 6.2 needs it for every single request to the PDF worker. The library restores the missing methods, so the viewer renders again.
- 30.0.0-rc.1 #3260 the draw and highlight editors accept new strokes again after a pinch zoom. Pinching used to hand the editor to the pinching finger, so the pen (or mouse) that was drawing before the gesture was ignored from then on, and re-selecting the tool didn't help. While a pen owns the editor, a finger scrolls the document again instead of doing nothing. Pinch zoom also no longer triggers on the tiniest two-finger movement.
- 30.0.0-rc.2 #3260 the draw and highlight editors no longer remember which pointer was drawing after you've closed the document. Until now the pen (or finger, or mouse) that had been given the editor kept it for as long as the page was open, so in a long session the editor could end up ignoring your finger, and opening another document didn't help. Every viewer now keeps track of this on its own, which is also a prerequisite for showing several viewers on one page.
- 30.0.0-rc.3 #3240 restoring annotations is quiet and predictable now. You get one `added` event per annotation, whatever its type - until now a restored highlight sent `added`, a restored free text sent a `moved` event for a move that never happened, and a restored drawing sent nothing at all. The viewer also stays in the editor mode it was in: adding a highlight programmatically opened the highlight editor and left it open - not on the first call, but on every one after it. Restored annotations were selected and given the keyboard focus as if you had pasted them, and a focused annotation asks the toolbar to switch to its editor's mode. And an annotation the viewer can't read no longer discards the annotations after it - it is skipped with a message naming its type. #3254 `addEditorAnnotation()` takes an array, and its TypeScript signature says so now. Pass a whole batch in one call: it is added in one step, so you get one undo step and one `added` event per annotation instead of a re-render per annotation. #3237 comments attached to annotations survive being restored. `popup` - the comment - is part of the annotation types now, so storing what `getSerializedAnnotations()` returns and feeding it back to `addEditorAnnotation()` type-checks; it was always in the data, only missing from the type. More importantly, restoring the comment actually works now: a comment on a free text annotation was dropped on the way back in, and a comment on a highlight came back but was gone again the next time you exported. While fixing that I found that a restored highlight also lost the stable `customId` of #3225 - it keeps it now, like the other annotation types always did. #3257 the digital signature button is visible again. It had no icon at all, so it was an invisible gap in the toolbar, and its panel opened past the right edge of the window. The button also no longer hides the top of your document: when it appears after the document has loaded it can push the toolbar onto a second row, and the viewer kept the height of the first row - which left just a sliver of the red "unverified signature" warning. Any toolbar button that shows up late benefits, not only this one. Re-creating the viewer - closing and re-opening it, or changing an input that reloads it - used to leave the signature UI behind for the rest of the session: the button disappeared and never came back, and the red warning bar disappeared with it. Both survive a reload now, and the button reflects the state your verifier reports each time. Dropping `[signatureVerifier]` also hides the button again, as documented; until now the previously supplied verifier stayed active. On top of that, the viewer detects zoneless change detection correctly now. It used to ask whether `zone.js` was loaded, which is a different question: Angular 22 is zoneless unless you add `provideZoneChangeDetection()`, while `zone.js` still sits in the `polyfills` array of every project generated before Angular 22. If that describes your application, the viewer never triggered change detection after asynchronous work - so things it learns late, like the signature warning bar, stayed invisible until an unrelated click redrew the page. The signature button also takes part in the responsive toolbar now: when the toolbar runs out of room it moves into the secondary menu like every other button, keeping the colour that tells you how the check went, and it opens the panel from there. And it no longer forces the toolbar onto a second row. Drawing with the ink editor works again. Starting a stroke raised an internal error, so no ink annotation was ever created - and because the error struck halfway through, it left a listener behind that then failed on the next click anywhere in the page ("Cannot read properties of null (reading 'isCancellable')"). The stroke that is abandoned without ever lifting the pointer no longer leaves that listener behind either.
- 30.0.0-rc.4 #3236 an annotation you add from your own code no longer disappears without a word when its `rect` is missing or malformed - you get a message in the console naming the annotation. Drawings are the exception and always were: they sit where their points are, so their `rect` is ignored. The showcase and the TypeScript types now say so, and explain `rotation`, which is not an angle to turn the annotation by: it says which way up the page was when you measured the coordinates. Pass the wrong one and your annotation lands in the wrong place rather than tilted - easy to run into with scanned documents, which are often stored sideways. #3264 a Content Security Policy that arrives as an HTTP header no longer sends you to the legacy bundle. To find out whether your browser can run the modern build, the viewer runs a small probe script; if your CSP blocks it because it is inline, the viewer used to conclude the browser was ancient and quietly load the much bigger `-es5` bundle instead - on every browser, however new. It now notices that the probe was blocked and loads it as a file. `[useInlineScripts]="false"` avoids the blocked attempt in the first place, and with it the violation your browser reports. If you provide Angular's `CSP_NONCE`, the viewer now also stamps it on the stylesheet pdf.js builds for printing, so a nonce-based policy keeps the paper size of your printout - and needs no `'unsafe-inline'` at all, neither for scripts nor for styles. The showcase now documents the smallest policy the viewer runs under, and what every directive in it is for.

...
- 28.0.0-alpha.0 updated pdf.js to v5.7.284 on the bleeding-edge branch (new merge-PDF UI plus the usual upstream bugfixes; new UI is gated by `pdfDefaultOptions.isBleedingEdge()`). #3115 stopped polluting the page-wide `color-scheme` — `[theme]` no longer fights host-application theming such as Angular Material auto/dark mode. #1321 fixed three initialization problems: a crash on viewer init, lost runtime reactivity of `[showUnverifiedSignatures]`, and a crash when a `pdfDefaultOptions` key was unknown to pdf.js. Removed the obsolete `isEvalSupported` default option (pdf.js no longer recognises it). #1321 also made `scrollPageIntoView()`, `[page]` and `[nameddest]` work on viewers that are temporarily hidden after init (route transitions, inactive tabs): the scroll is replayed when the viewer becomes visible again instead of being silently dropped with an `offsetParent is not set` warning. Added a "Hidden tabs & lazy routes" showcase page. Cleaned up obsolete troubleshooting docs (the long-removed `delayFirstView` attribute and the old "PDF viewer must be visible" workarounds — both unnecessary since v22.3.0). #3140 fixed the page thumbnails in the sidebar: the selected thumbnail's blue border is no longer clipped on the right, and thumbnails are centered horizontally. Fixed PDF JavaScript (form scripts, calculations) on sub-routes: the QuickJS WASM URL is now resolved correctly instead of 404-ing under `/your-route/wasm/`. #3141 added the missing "Add file" button to the thumbnail sidebar so the merge feature from pdf.js v6 is usable; enable it with `[enableMerge]="true"` or `pdfDefaultOptions.enableMerge = true`. Also exposed the "Current outline item" button and the sidebar heading label that pdf.js relies on for accessibility. #3142 fixed a crash when reordering pages by dragging thumbnails (without the split/merge feature enabled): pdf.js v6 added a checkbox to each thumbnail for the split/merge feature, but the drag-and-drop handler unconditionally accessed it, which crashed when split/merge was off.
